    ...
    html! {
        ...
        html lang="es" {
            head {
                meta charset="utf-8";
                meta name="viewport" content="width=device-width, initial-scale=1.0";
                title { "500 - Error del servidor | Tu Nombre" }
                meta http-equiv="X-Content-Type-Options" content="nosniff";
                meta http-equiv="X-Frame-Options" content="DENY";
                meta http-equiv="X-XSS-Protection" content="1; mode=block";
                script src="https://unpkg.com/htmx.org@2.0.3" integrity="sha384-0895/pl2MU10Hqc6jd4RvrthNlDiE9U1tWmX7WRESftEDRosgxNsQG/Ze9YMRzHq" crossorigin="anonymous" {}
                ...
                main class="error-main" {
                    section class="section error-section" {
                        div class="container" {
                            div class="content-card error-container" {
                                h1 { "500" }
                                h2 class="error-title" { "Error interno del servidor" }
                                p { "Ha ocurrido un error interno. Nuestro equipo ha sido notificado." }
                                p { "Por favor, inténtalo de nuevo en unos minutos." }
                                div class="error-button-container" {
                                    a href="/" class="cta-button" { "← Volver al inicio" }
                                }
                                div class="error-icon-container" { "⚠️" }
                            }
                        }
                    }
                }
